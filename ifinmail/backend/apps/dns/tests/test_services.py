...
def create_unmanaged_table(model: type) -> None:
    ...
    with (
        _patch.object(model._meta, 'managed', True),
...
class ProviderMapTests(SimpleTestCase):
...
class DNSServiceServerIPTests(SimpleTestCase):
    ...
    def test_get_server_ip_private_fallback_to_hostname(self, mock_get: MagicMock) -> None:
        ...
        with (
            patch.dict(os.environ, {'MAIL_HOSTNAME': 'mail.example.com'}),
...
class DNSServiceBuildRecordsTests(SimpleTestCase):
    ...
    @patch('builtins.open')
    def test_build_records_dkim_with_key(
        self, mock_open: MagicMock, mock_isfile: MagicMock
    ) -> None:
