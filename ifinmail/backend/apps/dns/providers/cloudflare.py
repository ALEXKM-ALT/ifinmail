...
class CloudflareProvider:
    ...
    def _get_zone_id(self, domain: str) -> str:
        ...
        if not zones:
            raise RuntimeError(
                f'No active Cloudflare zone found for {domain}. Add the domain to Cloudflare first.'
            )
        return zones[0]['id']
    ...
    def _existing_records(self, zone_id: str, domain: str) -> dict[tuple[str, str], str]:
        ...
        while True:
            data = self._request(
                'GET', f'/zones/{zone_id}/dns_records', params={'page': page, 'per_page': 100}
            )
    ...
    def configure_domain(self, domain: str, records: list[DNSRecord]) -> DNSResult:
        ...
        for rec in records:
            ...
            try:
                ...
                if key in existing:
                    ...
                    self._request(
                        'PUT', f'/zones/{zone_id}/dns_records/{existing[key]}', json=payload
                    )
        ...
        return DNSResult(
            ...
            message=f'Created/updated {len(created)} records'
