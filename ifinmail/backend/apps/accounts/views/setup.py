        ...
        with transaction.atomic():
            ...
            if not UserService.get_user_by_email(email):
                UserService.create_user(
                    email=email, password=password, is_active=True, is_staff=True
                )
    ...
    if step == 'welcome':
        ...
        if domain and server_ip and server_ip != '0.0.0.0':
            ...
            for rec in raw_records:
                ...
                if display_name and not display_name.startswith('_') or display_name:
...
def setup_advance(request: HttpRequest) -> HttpResponse:
    ...
    elif current == 'dns-auto' or current == 'dns-manual':
        next_step = 'account'
