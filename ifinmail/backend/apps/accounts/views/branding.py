...
_MEDIA_ROOT = getattr(django_settings, "MEDIA_ROOT", os.path.join(os.environ.get("APP_DIR", "/app"), "media"))
...
def branding_identity(request: HttpRequest) -> HttpResponse:
    ...
    secondary_color = overrides.get("secondary_color") or os.environ.get("BRAND_SECONDARY_COLOR", "#ECEEF0")
    ...
    if uploaded:
        ...
        with open(favicon_path, "wb+") as dest:
            for chunk in uploaded.chunks():
                dest.write(chunk)
        overrides["favicon_url"] = f"{getattr(django_settings, 'MEDIA_URL', '/media/')}branding/favicon.ico"
        changed = True
