...
class DeliverabilityService:
    ...
    def _check_dns_propagation(domain):
        ...
        checks = {
            ...
            "dkim": {"type": "TXT", "name": f"{dkim_selector}._domainkey.{domain}", "status": "unchecked", "detail": ""},
            "dmarc": {"type": "TXT", "name": f"_dmarc.{domain}", "status": "unchecked", "detail": ""},
        }
        ...
        return {
            ...
            "detail": "All DNS records resolve" if all_pass else "Some DNS records are not propagating",
            "fix": None if all_pass else "Verify DNS records were created. Propagation can take up to 48 hours.",
            "records": checks,
        }
    ...
    def _check_blacklists(server_ip):
        ...
        for rbl in rbls:
            ...
            try:
                ...
                answers = [l for l in result.stdout.strip().split("\n") if l and not l.startswith(";")]
                is_listed = len(answers) > 0 and "127." in result.stdout
                listings[rbl] = {"listed": is_listed, "detail": result.stdout.strip() if is_listed else "Not listed"}
                if is_listed:
                    listed_count += 1
            except Exception:
        ...
        if listed_count == 0:
            return {"status": "pass", "detail": "Not on any blacklists", "fix": None, "listings": listings}
        return {
            "status": "fail",
            "detail": f"Listed on {listed_count} blacklist(s)",
            "fix": "Visit the RBL's delisting page. Common causes: compromised account, open relay, or a previous owner sent spam.",
            "listings": listings,
        }
    ...
    def _check_reverse_dns(server_ip, mail_hostname):
        ...
        try:
            ...
            return {"status": "warn", "detail": f"rDNS check failed: {e}", "fix": "Verify your VPS provider supports PTR records."}

        if not ptr:
            return {
                "status": "fail",
                "detail": "No PTR record found for server IP",
                "fix": f"Contact your VPS provider and request a reverse DNS (PTR) record for {server_ip} pointing to {mail_hostname}.",
            }
        ...
        return {
            ...
            "fix": f"Contact your VPS provider and change the PTR record from {ptr} to {mail_hostname}.",
        }
    ...
    def _check_port25(mail_hostname):
        ...
        try:
            ...
            return {
                ...
                "fix": "Some VPS providers block port 25 by default. Contact your provider to unblock it.",
            }
        except ConnectionRefusedError:
            return {
                "status": "fail",
                "detail": f"Port {port} refused — Postfix may not be running",
                "fix": "Check that Postfix is running: docker compose ps postfix. If it's not, restart the stack.",
            }
        except OSError as e:
            return {"status": "fail", "detail": f"Port {port} unreachable: {e}", "fix": "Check firewall rules and VPS provider port 25 policy."}
    ...
    def _check_tls():
        ...
        if not cert_paths:
            return {"status": "fail", "detail": "No TLS certificate found — domain not configured", "fix": "Set DOMAIN and MAIL_HOSTNAME environment variables."}
        ...
        for cert_path in cert_paths:
            ...
            try:
                ...
                end_date = datetime.strptime(date_str, "%b %d %H:%M:%S %Y %Z").replace(tzinfo=timezone.utc)
                days = (end_date - datetime.now(timezone.utc)).days
                if days > 30:
                    return {"status": "pass", "detail": f"Valid for {days} days", "days": days, "fix": None}
                elif days > 7:
                    return {"status": "warn", "detail": f"Expires in {days} days", "days": days, "fix": "Certificate renews automatically. If it doesn't, check certbot container logs."}
                else:
                    return {"status": "fail", "detail": f"Expires in {days} days", "days": days, "fix": "Check certbot container: docker compose logs certbot"}
            except Exception:
                continue
