...
def _message_ids_query(days: int, user: User, db: Session):
    ...
    return (db.query(Message.id).filter(Message.mailbox_id.in_(mailbox_ids), Message.created_at >= since)).subquery()
...
def export_csv(
    ...
    for row in (
        db.query(Message.folder, sa_func.count(Message.id))
    ...
    for row in (
        db.query(sa_func.date(Message.created_at), sa_func.count(Message.id))
    ...
    opened = (
        db.query(sa_func.count(EmailDelivery.id))
        .filter(EmailDelivery.opened_at.isnot(None), EmailDelivery.created_at >= since)
        .join(Message, EmailDelivery.message_id == Message.id)
        .join(User, Message.mailbox_id == User.id)
    ...
    clicked = (
        db.query(sa_func.count(EmailDelivery.id))
        .filter(EmailDelivery.clicked_at.isnot(None), EmailDelivery.created_at >= since)
        .join(Message, EmailDelivery.message_id == Message.id)
        .join(User, Message.mailbox_id == User.id)
        .filter(User.id == user.id)
        .scalar()
    ...
    total_opens = (
        ...
        .filter(TrackingEvent.delivery_id.in_(db.query(delivery_ids.c.id)), TrackingEvent.event_type == "open")
        .scalar()
        or 0
    )
    total_clicks = (
        db.query(sa_func.count(TrackingEvent.id))
        .filter(TrackingEvent.delivery_id.in_(db.query(delivery_ids.c.id)), TrackingEvent.event_type == "click")
        .scalar()
        or 0
    )
    unique_opens = (
    ...
    for row in (
        ...
        .filter(TrackingEvent.delivery_id.in_(db.query(delivery_ids.c.id)), TrackingEvent.timestamp >= since)
    ...
    return Response(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=analytics_report_{datetime.now(UTC).strftime('%Y%m%d')}.csv"
        },
    )
...
def export_pdf(
    ...
    pdf.cell(
        0,
        8,
        f"Period: Last {days} days | Generated: {datetime.now(UTC).strftime('%Y-%m-%d %H:%M UTC')}",
        new_x="LMARGIN",
        new_y="NEXT",
        align="C",
    )
    ...
    total_msgs = db.query(sa_func.count(Message.id)).filter(Message.mailbox_id.in_(mailbox_ids)).scalar() or 0
    ...
    total_opens = (
        db.query(sa_func.count(TrackingEvent.id))
        .filter(TrackingEvent.delivery_id.in_(db.query(did.c.id)), TrackingEvent.event_type == "open")
        .scalar()
        or 0
    )
    unique_opens = (
    ...
    total_clicks = (
        db.query(sa_func.count(TrackingEvent.id))
        .filter(TrackingEvent.delivery_id.in_(db.query(did.c.id)), TrackingEvent.event_type == "click")
        .scalar()
        or 0
    )
    unique_clicks = (
    ...
    device_data = (
        db.query(TrackingEvent.device_type, sa_func.count(TrackingEvent.id))
        .filter(TrackingEvent.delivery_id.in_(db.query(did.c.id)), TrackingEvent.timestamp >= since)
        .group_by(TrackingEvent.device_type)
        .all()
    )
    ...
    loc_data = (
        db.query(TrackingEvent.city, TrackingEvent.country, sa_func.count(TrackingEvent.id))
        .filter(
            TrackingEvent.delivery_id.in_(db.query(did.c.id)),
            TrackingEvent.timestamp >= since,
            TrackingEvent.city.isnot(None),
            TrackingEvent.city != "",
        )
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=analytics_report_{datetime.now(UTC).strftime('%Y%m%d')}.pdf"
        },
    )
